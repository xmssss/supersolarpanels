...
public class NEIGenStone extends TemplateRecipeHandler {
	...
	public class GenStoneRecipe extends TemplateRecipeHandler.CachedRecipe {
		...
		public GenStoneRecipe(IRecipeInput container, IRecipeInput fill, RecipeOutput output1) {
			...
			this.ingredients.add(new PositionedStack(containerItems, 38 - 4, 10 - 4));
			this.ingredients.add(new PositionedStack(fillItems, 73 - 4, 10 - 4));
			this.output = new PositionedStack(output1.items.get(0), 56 - 4, 66 - 4);
		}
	}
	...
	public void drawBackground(int i) {
		...
		GuiDraw.drawTexturedModalRect(0, 0, 4, 4, 140, 78);
	}

	public void drawExtras(int i) {
		float f = (this.ticks >= 20) ? (((this.ticks - 20) % 20) / 20.0F) : 0.0F;

		drawProgressBar(51 - 4, 17 + 17 + 1 - 8 + 1 - 4, 176 + 1, 32, 25, 32, f, 1);
		f = (this.ticks <= 20) ? (this.ticks / 20.0F) : 1.0F;

		drawProgressBar(56 + 1 - 48 - 4, 20 + 14 + 2 - 4, 176, 0, 14, 14, f, 3);
	}
	...
	public void loadTransferRects() {
		this.transferRects.add(new TemplateRecipeHandler.RecipeTransferRect(new Rectangle(51, 17 - 4, 25, 32),
	...
	public void loadCraftingRecipes(String outputId, Object... results) {
		if (outputId.equals(getRecipeId())) {
			for (Map.Entry<IGenStoneRecipeManager.Input, RecipeOutput> entry : getRecipeList().entrySet())
				this.arecipes.add(new GenStoneRecipe(((IGenStoneRecipeManager.Input) entry.getKey()).container,
						((IGenStoneRecipeManager.Input) entry.getKey()).fill, entry.getValue()));
		} else {
	...
	public void loadUsageRecipes(ItemStack ingredient) {
		for (Map.Entry<IGenStoneRecipeManager.Input, RecipeOutput> entry : getRecipeList().entrySet()) {
			if (((IGenStoneRecipeManager.Input) entry.getKey()).container.matches(ingredient)
					|| ((IGenStoneRecipeManager.Input) entry.getKey()).fill.matches(ingredient))
				this.arecipes.add(new GenStoneRecipe(((IGenStoneRecipeManager.Input) entry.getKey()).container,
						((IGenStoneRecipeManager.Input) entry.getKey()).fill, entry.getValue()));
		}
	}
