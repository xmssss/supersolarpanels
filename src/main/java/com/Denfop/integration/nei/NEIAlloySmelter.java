...
public class NEIAlloySmelter extends TemplateRecipeHandler {
	...
	public void loadCraftingRecipes(String outputId, Object... results) {
		if (outputId.equals(getRecipeId())) {
			for (Map.Entry<IAlloySmelterRecipeManager.Input, RecipeOutput> entry : getRecipeList().entrySet())
				this.arecipes.add(new AlloySmelterRecipe(((IAlloySmelterRecipeManager.Input) entry.getKey()).container,
						((IAlloySmelterRecipeManager.Input) entry.getKey()).fill, entry.getValue()));
		} else {
		...
		for (Map.Entry<IAlloySmelterRecipeManager.Input, RecipeOutput> entry : getRecipeList().entrySet()) {
			for (ItemStack output : ((RecipeOutput) entry.getValue()).items) {
				if (NEIServerUtils.areStacksSameTypeCrafting(output, result))
					this.arecipes
							.add(new AlloySmelterRecipe(((IAlloySmelterRecipeManager.Input) entry.getKey()).container,
									((IAlloySmelterRecipeManager.Input) entry.getKey()).fill, entry.getValue()));
			}
		}
	}
	...
	public void loadUsageRecipes(ItemStack ingredient) {
		for (Map.Entry<IAlloySmelterRecipeManager.Input, RecipeOutput> entry : getRecipeList().entrySet()) {
			if (((IAlloySmelterRecipeManager.Input) entry.getKey()).container.matches(ingredient)
					|| ((IAlloySmelterRecipeManager.Input) entry.getKey()).fill.matches(ingredient))
				this.arecipes.add(new AlloySmelterRecipe(((IAlloySmelterRecipeManager.Input) entry.getKey()).container,
						((IAlloySmelterRecipeManager.Input) entry.getKey()).fill, entry.getValue()));
		}
	}
