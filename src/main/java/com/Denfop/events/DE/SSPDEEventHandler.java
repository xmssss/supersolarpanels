...
public class SSPDEEventHandler {

	@SubscribeEvent
	public void onPlayerInteract(LivingEvent.LivingUpdateEvent event) {
		if (event.entityLiving == null || !(event.entityLiving instanceof EntityPlayer) || !Config.Draconic)
			return;
		...
		for (int i = 0; i < player.inventory.mainInventory.length; i++) {
			...
			if (player.inventory.mainInventory[i] != null
				...
				if(Config.Draconic) {
				if (player.openContainer != null && player.openContainer instanceof ContainerDissEnchanter) {
