...
public class ItemSolarPanelHelmet extends ItemArmor implements IElectricItem, IMetalArmor, ISpecialArmor {
	...
	public void onArmorTick(World worldObj, EntityPlayer player, ItemStack itemStack) {
		...
		if (nbtData.getDouble("storage") >= maxstorage) {
			nbtData.setDouble("storage", this.maxstorage);
		}
		...
		this.tempstorage = nbtData.getDouble("storage");
		if (this.tempstorage > 0) {
			double energyLeft = this.tempstorage;
	...
	public void updateVisibility(EntityPlayer player) {
		this.wetBiome = (player.worldObj.getWorldChunkManager().getBiomeGenAt((int) player.posX, (int) player.posZ)
				.getIntRainfall() > 0);
		this.noSunWorld = player.worldObj.provider.hasNoSky;
		Boolean rainWeather = Boolean
				.valueOf((this.wetBiome && (player.worldObj.isRaining() || player.worldObj.isThundering())));
		if (!player.worldObj.isDaytime() || rainWeather.booleanValue()) {
			this.sunIsUp = false;
		} else {
			this.sunIsUp = true;
		}
		if (!player.worldObj.canBlockSeeTheSky((int) player.posX, (int) player.posY + 1, (int) player.posZ)
				|| this.noSunWorld) {
	...
	public void addInformation(final ItemStack itemStack, final EntityPlayer player, final List info, final boolean b) {
		...
		info.add(StatCollector.translateToLocal("ssp.storage.helmet") + " "
