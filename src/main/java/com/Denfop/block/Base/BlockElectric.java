										...
										.addChatMessage(
												new ChatComponentTranslation(
														StatCollector.translateToLocal("successfully.charged")
																+ String.valueOf(armorcharged.getDisplayName())
																+ StatCollector.translateToLocal("ssp.sendenergy")
				...
				for (ItemStack charged : entityPlayer.inventory.armorInventory) {
					if (charged != null) {
						...
						if (stack != null && stack.getItem() instanceof IEnergyContainerItem && tile.energy2 > 0) {
							...
							for (; item.getEnergyStored(charged) < item.getMaxEnergyStored(charged)
									&& tile.energy2 > 0;) {
								sent = (sent + tile.extractEnergy(null,
										item.receiveEnergy(stack, (int) tile.energy2, false), false));

								tile.setTransfer(tile.extractEnergy(null,
										item.receiveEnergy(stack, (int) tile.energy2, false), false) > 0);
							}
							if (sent > 0) {
								...
								entityPlayer
										.addChatMessage(
												new ChatComponentTranslation(
														StatCollector.translateToLocal("successfully.charged")
																+ String.valueOf(charged.getDisplayName())
																+ StatCollector.translateToLocal("ssp.sendenergy")
		...
		if (!entityPlayer.isSneaking()) {
			...
			if (tileentity != null) {
				...
				if (world.getTileEntity(x, y, z) instanceof TileEntityElectricBlock) {
					...
					if (tile.personality && (tile.UUID == entityPlayer.getDisplayName() || entityPlayer.capabilities.isCreativeMode)) {
