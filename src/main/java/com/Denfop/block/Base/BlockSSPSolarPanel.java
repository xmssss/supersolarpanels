	...
	public boolean onBlockActivated(final World world, final int i, final int j, final int k, final EntityPlayer player,
		...
		if (tileentity != null) {
			...
			if (world.getTileEntity(i, j, k) instanceof TileEntitySolarPanel) {
				...
				for (int m = 0; m < 9; m++) {
					if (tile.chargeSlots[m] != null && tile.chargeSlots[m].getItem() instanceof AdditionModule
							&& tile.chargeSlots[m].getItemDamage() == 0 && (tile.player == player.getDisplayName() || player.capabilities.isCreativeMode)) {
